...
void intro()
{
	std::cout << std::endl
	<< " +--------------------------------------------------------------------------+" << std::endl
	<< " |                       SimpleYggGen C++ 1.0-hothead                       |" << std::endl
...
std::string getAddress(unsigned char HashValue[SHA512_DIGEST_LENGTH])
{
	...
	for(int i = 0; i < 14; ++i)
	{
		if(HashValue[i] == 0)	// большая вложенность if-ов (не одним условием) для большей производительности
		{
			if(HashValue[i+1] == 0)
			{
				if(i % 2 == 0)
				{
					if(HashValue[i+2] == 0 && HashValue[i+3] == 0)
					{
						ss << ":";
						i += 3;
					} else {
						ss << "0";
						++i;
						if(i != 13)	// не выводим двоеточие в конце адреса
							ss << ":";
					}
				}
			}
		} else if(HashValue[i] == 0 && i % 2 == 0) { // не выводим первые нулевый байты секции
		} else {
			if(i % 2 == 0 || ((HashValue[i-1] == 0 && i % 2 != 0)))	// если выводится первый байт секции, нули сокращаем
